...
function install_shellcheck {
  ...
  #Differently named packages for pkg-config
  #if [[ "$PACKAGE_MANAGER" == "brew" ]] || [[ "$PACKAGE_MANAGER" == "apk" ]] || [[ "$PACKAGE_MANAGER" == "apt-get" ]] || [[ "$PACKAGE_MANAGER" == "pacman" ]]; then
  #  install_pkg shellcheck "$PACKAGE_MANAGER"
  #fi
  #if [[ "$PACKAGE_MANAGER" == "yum" ]]; then
  #  PRE_COMMAND=()
  #  if [ "$(whoami)" != 'root' ]; then
  #    PRE_COMMAND=(sudo)
  #  fi
  #  "${PRE_COMMAND[@]}" yum -y install epel-release
  #  install_pkg ShellCheck "$PACKAGE_MANAGER"
  #fi
}
