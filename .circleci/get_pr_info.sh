...
elif  [[ ! -z  "${CIRCLE_PROJECT_REPONAME}" ]] && [[ ! -z  "${CIRCLE_PROJECT_USERNAME}" ]]; then
...
if [[ ! -z "$USERSET_PREVIOUS_GITHASH" ]] && git merge-base --is-ancestor ${USERSET_PREVIOUS_GITHASH} $(git rev-parse HEAD) 2>/dev/null ; then
...
if [[ ! -z ${PR_NUMBER} ]]; then
  ...
  if [[ ! -z ${PR_RESPONSE} ]] && [[ `echo "${PR_RESPONSE}" | jq -e '.message'` != '"Not Found"' ]]; then
  ...
  if [[ ! -z "${PR_BASE_BRANCH}" ]] && [[ `git branch -l --no-color | tr -d '*' | tr -d ' ' | grep '^'"${PR_BASE_BRANCH}"'$' | wc -l` == 1 ]]; then
    BASE_GITHASH=`git merge-base HEAD origin/${PR_BASE_BRANCH}`
    TARGET_BRANCH=$PR_BASE_BRANCH
  fi
fi

if [[ ! -z "${BASE_GITHASH}" ]]; then
